...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    // COMPATIBILIDADE (fase bcb, fundação): o worker artesanal anterior servia
    // o JSON-RPC do MCP em `POST /`, e é isso que o README publicou desde
    // sempre. O template da Fase 0 serve em /mcp. Sem esta reescrita, todo
    // cliente HTTP já configurado quebraria em silêncio no dia do retrofit.
    // GET / continua sendo a landing page.
    if (url.pathname === "/" && request.method === "POST") {
      url = new URL(SERVER_CONFIG.mcpRoute, url);
      request = new Request(url, request);
      record("legacy_root_post", SERVER_CONFIG.mcpRoute);
    }
    ...
    //
    // POR QUE DEPOIS DO HANDLER, e não antes como no ilo-mcp-server. Aqui quem
    // valida `Host` e `Origin` é o próprio `createMcpHandler`; um guarda
    // colocado antes dele responderia -32602 a uma requisição que a checagem de
    // segurança ia recusar com 403 — recusa de protocolo passando à frente da
    // recusa de segurança. Rodando depois, só substituímos uma resposta que já
    // passou por Host, Origin, auth e rate limit. O corpo é lido de uma CÓPIA
    // tirada ANTES do handler (depois dele o stream já foi consumido).
    ...
    // (A recusa de cursor acima também é lida daqui: o corpo dela carrega o
    // -32602, e o envelope o classifica como `contrato` sem caso especial.)
    ...
    logger.info("request", {
      ...
      ms: Date.now() - start,
      ...(recusaDeCursor ? { invalid_cursor: true } : {}),
    });
    return response;
  },
