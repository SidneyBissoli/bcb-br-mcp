...
const SSE = {
  recusaDeEsquema:
    'event: message\ndata: {"result":{"content":[{"type":"text","text":"Input validation error: Invalid arguments for tool bcb_serie_ultimos: #: Property \\"codigo\\" does not match schema.; #/codigo: Instance type \\"string\\" is invalid. Expected \\"number\\"."}],"isError":true},"jsonrpc":"2.0","id":1}\n\n',
  toolInexistente:
  ...
  erroMole:
    'event: message\ndata: {"result":{"content":[{"type":"text","text":"{\\"resultados\\": [], \\"aviso\\": \\"Nenhuma serie do catalogo casa com o termo.\\"}"}]},"jsonrpc":"2.0","id":1}\n\n',
...
describe("LeitorDeEnvelope — corpos medidos na produção", () => {
  ...
  it("ERRO-MOLE continua sendo acerto: ausência respondida como dado não é falha", () => {
    // Busca que nao casa nada responde lista vazia numa resposta de SUCESSO —
...
describe("a linha que iria para o Analytics Engine", () => {
  ...
  it("recusa de esquema, HTTP 200: error + classe contrato (antes NÃO HAVIA LINHA)", async () => {
    ...
    const linha = await umaLinha(chamada("bcb_serie_ultimos", { query: 12345 }), SSE.recusaDeEsquema);
    expect(linha.blobs?.[0]).toBe("bcb_serie_ultimos");
  ...
  it("ausência respondida como dado segue ok e SEM classe", async () => {
    const linha = await umaLinha(chamada("bcb_serie_ultimos", { query: "zzzqqq" }), SSE.erroMole);
  ...
  it("chamada que o hook JÁ GRAVOU não gera linha aqui: a reconciliação fecha", async () => {
    ...
    const pontos = await linhas(
      chamada("bcb_serie_valores", {}, 7),
      SSE.sucesso,
      200,
      new Map([["bcb_serie_valores", 1]]),
    );
    expect(pontos).toEqual([]);
  });

  it("chamada válida sem recibo segue ok, e o resto da linha não muda", async () => {
    const linha = await umaLinha(chamada("bcb_indicadores_atuais", {}, 7), SSE.sucesso);
    expect(linha.blobs?.[1]).toBe("ok");
  ...
  it("lote: cada mensagem recebe o desfecho DELA, não o do vizinho", async () => {
    const pontos = await linhas(
      [chamada("bcb_serie_valores", {}, 1), chamada("nao_existe", {}, 2)],
